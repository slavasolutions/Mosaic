 ...
 *
 * Port of `packages/astro/src/url.ts`. Kept verbatim in spirit so two
 * adapters agree on URL derivation. When a third adapter lands we'll
 * factor this out into a shared module; premature for now.
 */
...
/**
 * Convert a Mosaic URL (e.g. `/about` or `/blog/hello`) into the
 * `slug: string[]` shape Next App Router's catch-all `[[...slug]]`
 * expects for `generateStaticParams`. The root URL `/` becomes `[]`.
 */
export function urlToSlugArray(url: string): string[] {
  return segments(url);
}

/**
 * Inverse of {@link urlToSlugArray}: rebuild the URL from a `slug` array
 * (or `undefined` for the optional catch-all root). Empty array / undefined
 * returns `/`.
 */
export function slugArrayToUrl(slug: string[] | undefined): string {
